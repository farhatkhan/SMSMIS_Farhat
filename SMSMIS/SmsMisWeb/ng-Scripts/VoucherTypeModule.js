(function () {
    ...
    var comController = function ($scope, $http) {
        ...
        $scope.onCompanyChange = function () {
            ...
            $http.post("/UI/getAllVoucherTypeCompany", { CompanyCode: $scope.selectedObject.CompanyCode }).then(onListComplete, onRequestError);
        }
        ...
        var isValid = function () {
            ...
            else if (isnullorEmpty($scope.selectedObject.VoucherName))
                return false;//$scope.listError = "Voucher Name is Required";
            else if (isnullorEmpty($scope.selectedObject.ShortName))
                return false;//$scope.listError = "Short Name is Required";
            else if (isnullorEmpty($scope.selectedObject.Category))
                return false;//$scope.listError = "Category is Required";
            else if (isnullorEmpty($scope.selectedObject.Frequency))
                return false;//$scope.listError = "Frequency is Required";
        ...
        $scope.save = function () {
            ...
            $http.post("/UI/saveVoucherType", { VoucherType: $scope.selectedObject, companyId: $scope.selectedObject.CompanyCode }).then(onSaveComplete, onRequestError);//saif
        }
        ...
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                showDeleteShield();
                $http.post("/UI/deleteVoucherType", $scope.selectedObject).then(onDelete, onRequestError)
            }
        }
        ...
        $scope.clear = function () {
            ...
            $scope.selectedObject = { Category: 'J', Frequency: 'M', Status: '1', CompanyCode: ccode };
        }
        $scope.listData = [];
        $scope.listCompany = [];
        $scope.isEditMode = false;
        $scope.selectedObject = { Category: 'J', Frequency: 'M', Status: '1' };
    ...
    app.controller("VoucherTypeController", comController);
}
)();