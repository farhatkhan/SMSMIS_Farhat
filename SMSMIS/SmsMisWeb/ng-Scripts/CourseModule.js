(function () {
    ...
    var couController = function ($scope, $http,$filter) {
        ...
        $scope.isValid = function () {
            return ((myForm.CourseName.value == '')
        ...
        $scope.save = function () {
            showShield();
            $scope.saveError = '';
            $http.post("/UI/saveCourse", $scope.selectedObject).then(onSaveComplete, onRequestError)
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                showShield();
                $http.post("/UI/deleteCourse", $scope.selectedObject).then(onDelete, onRequestError)
            }
            hideShield();
        }
        $scope.isEditMode = false;
        ...
        $scope.saveError = '';
        showShield();
        $http.get("/UI/getAllCourses").then(onListComplete, onRequestError);
        $http.get("/UI/getAllActiveCompanies").then(onGetCompany, onRequestError);
        hideShield();
    }

    app.controller("courseController", couController);
}
)();