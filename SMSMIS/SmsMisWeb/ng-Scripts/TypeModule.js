(function () {
    ...
    var couController = function ($scope, $http, $filter) {
        ...
        $scope.CompanyCode_Change = function () {
            //if (!$scope.isEditMode) {
            showShield();
            $scope.selectedObject.ShortName = $scope.selectedObject.TypeName = '';
            $scope.isEditMode = false;
            $http.post("/UI/getAllType", { CompanyCode: $scope.selectedObject.CompanyCode }).then(onListComplete, onRequestError);
        ...
        $scope.load = function (obj) {
            ...
            if (obj != null) {
                ...
                for (var x in $scope.selectedObject.TypeBranch) {

                    var vBranchCode = $scope.selectedObject.TypeBranch[x].BranchCode;
        ...
        $scope.save = function () {
            $scope.selectedObject.TypeBranch = GetSelectedBranches();
            if ($scope.selectedObject.TypeBranch == null || $scope.selectedObject.TypeBranch.length < 1)
                return;
            showSaveShield();
            $scope.saveError = '';
            ...
            //$scope.isEditMode ? $scope.selectedObject.TypeCode:
            $http.post("/UI/saveType", { 'Type': $scope.selectedObject, 'isNew': $scope.isEditMode ? false : true }).then(onSaveComplete, onRequestError);
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?'))
                $http.post("/UI/deleteType", $scope.selectedObject).then(onDelete, onRequestError)
            hideAllShield();
        }
        $scope.isEditMode = false;
        $scope.clear();
    ...
    app.controller("typeController", couController);
}
)();