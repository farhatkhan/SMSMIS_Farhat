(function () {
    ...
    var subController = function ($scope, $http) {
        ...
        $scope.save = function () {
            showShield();
            $scope.saveError = '';
            $http.post("/UI/saveDocType", $scope.selectedObject).then(onSaveComplete, onRequestError)
            hideShield();
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                showShield();
                $http.post("/UI/deleteDocType", $scope.selectedObject).then(onDelete, onRequestError)
            }
            hideShield();
        }
        ...
        $scope.saveError = '';
        showShield();
        $http.get("/UI/getAllDocType").then(onListComplete, onRequestError);
        hideShield();
    }

    app.controller("doctypeController", subController);
}
)();