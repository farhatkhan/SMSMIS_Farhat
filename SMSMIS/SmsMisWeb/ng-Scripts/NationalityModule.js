(function () {
    ...
    var subController = function ($scope, $http) {
        ...
        $scope.isValid = function () {
            return ((myForm.NationalityName.value == '')
        ...
        $scope.save = function () {
            showShield();
            $scope.saveError = '';
            $http.post("/UI/saveNationality", $scope.selectedObject).then(onSaveComplete, onRequestError)
        }
        $scope.delete = function () {
            $scope.saveError = '';
            if (confirm('Are you sure you want to delete this record?')) {
                showShield();
                $http.post("/UI/deleteNationality", $scope.selectedObject).then(onDelete, onRequestError)
            }
        }
        ...
        $scope.clear();
        $scope.saveError = '';
        $http.get("/UI/getAllNationality").then(onListComplete, onRequestError);
        hideShield();
    }

    app.controller("nationalityController", subController);
}
)();