(function () {
    ...
    var admController = function ($scope, $http,$filter) {
        ...
        $scope.getCompanyWise = function () {
            ...
            $http.post("/UI/getAllStudentFixedDiscountTypeActive", { 'CompanyCode': $scope.selectedObject.CompanyCode }).then(onGetDiscountType, onRequestError);
            $http.post("/UI/getAllStudentFixedDiscount", { 'CompanyCode': $scope.selectedObject.CompanyCode }).then(onGetAllData, onRequestError);
            $scope.showGrid();
        }
        $scope.showGrid = function () {
            $scope.selectedObject.DiscountAmount = $scope.selectedObject.DiscountPercentage = '';
            $scope.filteredArray = {};
        ...
        var onGetDiscountType = function (response) {
            if (response.data == null || typeof response.data == 'undefined') listData = true;
            $scope.listDiscount = response.data;
            GetAllDiscount = true;
            isFormLoaded();
        }
        //saifhttp://stackoverflow.com/questions/19326837/angularjs-filter-by-multiple-models
        ...
        $scope.save = function () {
            clearErros();
            if (angular.isUndefined($scope.selectedObject.DiscountAmount) || $scope.selectedObject.DiscountAmount == '')
                $scope.selectedObject.DiscountAmount = 0;
            if (angular.isUndefined($scope.selectedObject.DiscountPercentage) || $scope.selectedObject.DiscountPercentage == '')
                $scope.selectedObject.DiscountPercentage = 0;
            if (!isValid()) return;
            $scope.selectedObject.DiscountPercentage = parseFloat($scope.selectedObject.DiscountPercentage).toFixed(3);
            $scope.selectedObject.DiscountAmount = parseFloat($scope.selectedObject.DiscountAmount).toFixed(3);
            ...
            if ($scope.selectedObject != null && typeof $scope.selectedObject != 'undefined' && $scope.selectedObject.StudentNo > 0) {
                ...
                $http.post("/UI/saveStudentFixedDiscount", { StudentFixedDiscount: $scope.selectedObject, isNew: isNew }).then(onSaveComplete, onRequestError);//saif
            }
        }
        var isValid = function () {
            if (isnullorEmpty($scope.selectedObject.DiscountPercentage)) $scope.selectedObject.DiscountPercentage = 0;
            if (isnullorEmpty($scope.selectedObject.DiscountAmount )) $scope.selectedObject.DiscountAmount = 0;
            ...
            else if (isnullorEmpty($scope.selectedObject.DiscountTypeCode))
                return false;//$scope.listError = "Session is Required";
            else if (isnullorEmpty($scope.selectedObject.StudentNo))
                return false;//$scope.listError = "Student is Required";
            else if (parseInt($scope.selectedObject.DiscountPercentage) == 0 && parseInt($scope.selectedObject.DiscountAmount) == 0)
                return false;//$scope.listError = "Discount (%) is Required";
            else if (parseInt($scope.selectedObject.DiscountAmount) > 0 && parseInt($scope.selectedObject.DiscountPercentage) > 0)
                return false;
        ...
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?'))
                $http.post("/UI/deleteStudentFixedDiscount", $scope.selectedObject).then(onDelete, onRequestError)
        }
        ...
        var isFormLoaded = function () {
            if (GetAllData && GetCompany && GetAllSession && GetAllBranch && GetAllStudent && GetAllDiscount) {
                hideAllShield();
            }
        }
    ...
    app.controller("StudentFixedDiscountController", admController);
}
)();