(function () {
    ...
    var couController = function ($scope, $http, $filter) {
        ...
        $scope.CompanyCode_Change = function () {
            $scope.selectedObject.SectionName = $scope.selectedObject.ShortName = $scope.selectedObject.Capacity = '';
            $scope.selectedObject.Status = true;
        ...
        $scope.isValid = function () {
            ...
            return ((myForm.SectionName.value == '')
            || (myForm.ShortName.value == '')
            || (myForm.Capacity.value == '')
        ...
        $scope.save = function () {
            showShield();
            $scope.saveError = '';
            $http.post("/UI/saveSection", $scope.selectedObject).then(onSaveComplete, onRequestError)
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                showShield();
                $http.post("/UI/deleteSection", $scope.selectedObject).then(onDelete, onRequestError)
            }
            hideShield();
        }
        $scope.isEditMode = false;
        ...
        $scope.saveError = '';
        showShield();
        $http.get("/UI/getAllSections").then(onListComplete, onRequestError);
        $http.get("/UI/getAllActiveCompanies").then(onGetCompany, onRequestError);
        hideShield();
    }

    app.controller("sectionController", couController);
}
)();