(function () {
    ...
    var couController = function ($scope, $http, $filter) {
        ...
        $scope.CompanyCode_Change = function () {
            //if (!$scope.isEditMode) {
            showShield();
            $scope.selectedObject.ShortName = $scope.selectedObject.CategoryName = '';
            $scope.isEditMode = false;
            $http.post("/UI/getAllCategory", { CompanyCode: $scope.selectedObject.CompanyCode }).then(onListComplete, onRequestError);
        ...
        $scope.isValid = function () {
            return ((myForm.CategoryName.value == '')
        ...
        $scope.load = function (obj) {
            ...
            if (obj != null) {
                ...
                for (var x in $scope.selectedObject.CategoryBranch) {

                    var vBranchCode = $scope.selectedObject.CategoryBranch[x].BranchCode;
        ...
        $scope.save = function () {
            $scope.selectedObject.CategoryBranch = GetSelectedBranches();
            if ($scope.selectedObject.CategoryBranch == null || $scope.selectedObject.CategoryBranch.length < 1)
                return;
            showSaveShield();
            $scope.saveError = '';

            //$scope.selectedObject.CategoryBranch = GetSelectedBranches();
            //$scope.isEditMode ? $scope.selectedObject.CategoryCode:
            $http.post("/UI/saveCategory", { 'Category': $scope.selectedObject, 'isNew': $scope.isEditMode ? false : true }).then(onSaveComplete, onRequestError);
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?'))
                $http.post("/UI/deleteCategory", $scope.selectedObject).then(onDelete, onRequestError)
            hideAllShield();
        }
        $scope.isEditMode = false;
        $scope.clear();
    ...
    app.controller("categoryController", couController);
}
)();