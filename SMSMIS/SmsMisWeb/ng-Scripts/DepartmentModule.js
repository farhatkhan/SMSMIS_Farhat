(function () {
    ...
    var couController = function ($scope, $http, $filter) {
        ...
        $scope.CompanyCode_Change = function () {
            //if (!$scope.isEditMode) {
            showShield();
            $scope.selectedObject.ShortName = $scope.selectedObject.DepartmentName = '';
            $scope.isEditMode = false;
            $http.post("/UI/getAllDepartment", { CompanyCode: $scope.selectedObject.CompanyCode }).then(onListComplete, onRequestError);
        ...
        $scope.isValid = function () {
            return ((myForm.DepartmentName.value == '')
        ...
        $scope.load = function (obj) {
            ...
            if (obj != null) {
                ...
                for (var x in $scope.selectedObject.DepartmentBranch) {

                    var vBranchCode = $scope.selectedObject.DepartmentBranch[x].BranchCode;
        ...
        $scope.save = function () {
            $scope.selectedObject.DepartmentBranch = GetSelectedBranches();
            if ($scope.selectedObject.DepartmentBranch == null || $scope.selectedObject.DepartmentBranch.length < 1)
                return;
            showSaveShield();
            $scope.saveError = '';

            //$scope.selectedObject.DepartmentBranch = GetSelectedBranches();
            //$scope.isEditMode ? $scope.selectedObject.DepartmentCode:
            $http.post("/UI/saveDepartment", { 'Department': $scope.selectedObject, 'isNew': $scope.isEditMode ? false : true }).then(onSaveComplete, onRequestError);
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?'))
                $http.post("/UI/deleteDepartment", $scope.selectedObject).then(onDelete, onRequestError)
            hideAllShield();
        }
        $scope.isEditMode = false;
        $scope.clear();
    ...
    app.controller("departmentController", couController);
}
)();