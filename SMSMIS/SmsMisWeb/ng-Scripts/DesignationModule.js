(function () {
    ...
    var couController = function ($scope, $http, $filter) {
        ...
        $scope.CompanyCode_Change = function () {
            //if (!$scope.isEditMode) {
            showShield();
            $scope.selectedObject.ShortName = $scope.selectedObject.DesignationName = '';
            $scope.isEditMode = false;
            $http.post("/UI/getAllDesignation", { CompanyCode: $scope.selectedObject.CompanyCode }).then(onListComplete, onRequestError);
        ...
        $scope.isValid = function () {
            return ((myForm.DesignationName.value == '')
        ...
        $scope.load = function (obj) {
            ...
            if (obj != null) {
                ...
                for (var x in $scope.selectedObject.DesignationBranch) {

                    var vBranchCode = $scope.selectedObject.DesignationBranch[x].BranchCode;
        ...
        $scope.save = function () {
            $scope.selectedObject.DesignationBranch = GetSelectedBranches();
            if ($scope.selectedObject.DesignationBranch == null || $scope.selectedObject.DesignationBranch.length < 1)
                return;
            showSaveShield();
            $scope.saveError = '';

            //$scope.selectedObject.DesignationBranch = GetSelectedBranches();
            //$scope.isEditMode ? $scope.selectedObject.DesignationCode:
            $http.post("/UI/saveDesignation", { 'Designation': $scope.selectedObject, 'isNew': $scope.isEditMode ? false : true }).then(onSaveComplete, onRequestError);
        }
        $scope.delete = function () {
            ...
            if (confirm('Are you sure you want to delete this record?'))
                $http.post("/UI/deleteDesignation", $scope.selectedObject).then(onDelete, onRequestError)
            hideAllShield();
        }
        $scope.isEditMode = false;
        $scope.clear();
    ...
    app.controller("designationController", couController);
}
)();